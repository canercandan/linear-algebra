...
namespace linear_algebra
{
    ...
    Logger::Logger()
	: std::ostream(&_obuf),
	  _selectedLevel(linear_algebra::progress), _contextLevel(linear_algebra::quiet),
	...
	addLevel("quiet", linear_algebra::quiet);
	addLevel("errors", linear_algebra::errors);
	addLevel("warnings", linear_algebra::warnings);
	addLevel("progress", linear_algebra::progress);
	addLevel("logging", linear_algebra::logging);
	addLevel("debug", linear_algebra::debug);
	addLevel("xdebug", linear_algebra::xdebug);
    }
    ...
    void	Logger::addLevel(std::string name, linear_algebra::Levels level)
    {
	_levels[name] = level;
    ...
    Logger&	operator<<(Logger& l, const linear_algebra::Levels lvl)
    {
	l._contextLevel = lvl;
	return l;
    }

    Logger&	operator<<(Logger& l, linear_algebra::file f)
    {
	l._fd = ::open(f._f.c_str(), O_WRONLY | O_APPEND | O_CREAT, 0644);
	return l;
    }

    Logger&	operator<<(Logger& l, linear_algebra::setlevel v)
    {
    ...
    Logger::outbuf::outbuf(const int& fd,
			     const linear_algebra::Levels& contexlvl,
			     const linear_algebra::Levels& selectedlvl)
